...
def eval_runner_isles():
    ...
    for model in models:
        ...
        if Path(f"{model_name}_metrics.json").exists():
            print(f"Skipping {model.name}, metrics already computed.")
            continue
        ...
        for pred_file_path in (model / "validation").iterdir():
            ...
            gt_file_path = (
                ...
                / f"{pred_file_name}"
                / "ses-0001"
                / f"{pred_file_name}_ses-0001_msk.nii.gz"
            )
    ...
    # Save to CSV
    metrics_csv = current_dir / "isles_metrics.csv"
