...
def eval_msd_brats():
    ...
    models = [d for d in current_dir.iterdir() if d.is_dir() and d.name != "gt" and "Gram" in d.name]

    for model in models:
        model_name = model.name

        if Path(f"{model_name}_metrics.json").exists():
            print(f"Metrics for {model_name} already exist, skipping evaluation.")
            continue
    ...
    # get all the *_metrics.json files in the current directory and make a dataframe with class names + average as columns
    # and model_name as index. then save it to a csv file as well.

    # class specific metrics are under key: "mean": {"1": {"metric_name": value, ...}, "2": ..., ...}
    # global metrics are under key: "foreground_mean": {"metric_name": value, ...}
