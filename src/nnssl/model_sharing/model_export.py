    ...
    with(zipfile.ZipFile(output_file, 'w', zipfile.ZIP_DEFLATED)) as zipf:
        for c in configurations:
            ...
            for fold_folder in expected_fold_folder:
                ...
                if export_crossval_predictions:
                    ...
                    for f in files:
                        zipf.write(join(source_folder, f), os.path.relpath(join(source_folder, f), nnssl_results))
        ...
        for ens in subd:
            ...
            if ok:
                ...
                for s in source_files:
                    zipf.write(s, os.path.relpath(s, nnssl_results))
        inference_information_file = join(nnssl_results, dataset_name, 'inference_information.json')
        if isfile(inference_information_file):
            zipf.write(inference_information_file, os.path.relpath(inference_information_file, nnssl_results))
        inference_information_txt_file = join(nnssl_results, dataset_name, 'inference_information.txt')
        if isfile(inference_information_txt_file):
            zipf.write(inference_information_txt_file, os.path.relpath(inference_information_txt_file, nnssl_results))
    print('Done')
